...
- ``type``: Use ``GG`` to have ``pfweight`` read gravity gradiometry files.

- ``mesh.msh``: Name of 3D :ref:`mesh file <meshFile>`.

- ``obs.gg`` The :ref:`data file <ggFile>` that contains the observation locations and the observed vertical gravity anomaly with estimated standard deviation.
